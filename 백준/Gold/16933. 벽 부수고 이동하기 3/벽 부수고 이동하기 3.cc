...
void V() {
	...
	while (!q.empty()) {
		...
		int x = q.front().first.first.second, y = q.front().first.first.first;
		int P = q.front().first.second.second, R = q.front().first.second.first; q.pop();
		...
		for (int n = 0; n < 4; n++) {
			...
			if (T) {
				if (R == 0) {
					r[Y][X][0] = 1;
					if (v[Y][X]) { q.push({ { {Y,X},{1,P + 1} } ,0 }); }
					else { q.push({ { {Y,X},{0,P + 1} } ,0 }); }
				}
				else if (!v[Y][X]) {
					r[Y][X][R] = 1;
					q.push({ {{Y,X},{R,P + 1}},0 });
