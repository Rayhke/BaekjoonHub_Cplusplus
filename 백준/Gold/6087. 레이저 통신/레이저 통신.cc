...
priority_queue <pair<PairIS, PairSS>, vector<pair<PairIS, PairSS>>, greater<pair<PairIS, PairSS>>> pq;

void V() {
	while (!pq.empty()) {
		short x = pq.top().second.second, y = pq.top().second.first, z = pq.top().first.second;
		int P = pq.top().first.first; pq.pop();
		for (int n = 0; n < 4; n++) {
			short X = x + PM[n].second, Y = y + PM[n].first;
			if (X < 0 || W <= X || Y < 0 || H <= Y || r[Y][X]) { continue; }
			if (P != -1) {
				if (z != n) {
					if (v[Y][X][n] > P + 1) { m[Y][X][n] = 1; v[Y][X][n] = P + 1; pq.push({ {P + 1,n},{Y,X} }); }
				}
				else {
					if (v[Y][X][n] > P) { m[Y][X][n] = 1; v[Y][X][n] = P; pq.push({ {P,n},{Y,X} }); }
				}
			}
			else {
				if (v[Y][X][n] > P) { m[Y][X][n] = 1; v[Y][X][n] = P; pq.push({ {P,n},{Y,X} }); }
